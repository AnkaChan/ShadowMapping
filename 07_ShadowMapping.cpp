...
void keyCallBack(GLFWwindow* window, int key, int scancode, int action, int mods) {
	if (key == GLFW_KEY_F6 && action == GLFW_RELEASE)
	{
		...
		programRender.BuildFiles(inputVertexShader, inputFragmetsShader);
...
int main(int argc, char ** argv)
{
	...
	do {
		...
		// in the "MVP" uniform
		programDepthMap.SetUniformMatrix4("MVP", &depthMVP[0][0]);
		programDepthMap.SetUniformMatrix4("V", &depthViewMatrix[0][0]);
		programDepthMap.SetUniformMatrix4("M", &depthModelMatrix[0][0]);
		programDepthMap.SetUniform("LightPosition_worldspace", LightPosition_worldspace.x, LightPosition_worldspace.y, LightPosition_worldspace.z);
		...
		programRender.SetUniformMatrix4("DepthMapMVP", &depthMVP[0][0]);
